...
Islam et al. show that handwritten source-code OCR is unusually brittle: minor transcription or indentation errors can make code non-executable. Their evaluation separates transcription fidelity from “logical fix hallucination,” because a model that silently repairs a student’s logic may undermine faithful assessment and learning. PaperCode v1.1 retains raw transcriptions and removes the previous digit/letter substitution heuristics. Normalization covers mathematical typography, x/y Unicode lookalikes, and single-letter variable case, with a visible change record. Ambiguous readings remain marked for review and displayed results are provisional. DOI: [10.1145/3657604.3662027](https://doi.org/10.1145/3657604.3662027).
...
The interpreter is deterministic and covered by automated tests. Handwriting mode uses Apple Vision request revision 3; its model is supplied by macOS and cannot be treated as a repository-pinned model. Record the full macOS build, hardware, browser, request settings, camera, lighting, pen, paper, and application commit. Tesseract 5.1.1 remains bundled for printed-text mode only. See [OCR_VALIDATION.md](OCR_VALIDATION.md) for the limited development evidence, known failures, and a reproducible evaluation command.
